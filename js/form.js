...
$(document).ready(function() {
  ...
  $('#staveform').submit(function(event) {
    ...
    s.shell_diameter = parseInt($.trim($('#shell_diameter').val()));
    ...
    /** SET DISPLAY VALUES **/
    $('#display_shell_diameter').html(s.shell_diameter.toFixed(3) + '"');
    $('#display_finished_diameter').html((s.shell_diameter.toFixed(3) - 0.125) + '"');
    $('#display_shell_depth').html(s.shell_depth.toFixed(1) + '"');
    $('#display_stave_count').html(s.stave_count);
    $('#display_board_thickness').html(s.board_thickness.toFixed(3) + '"');
    $('#display_rounded_thickness').html(s.roundedThickness().toFixed(3) + '"');
    $('#display_staves_per_width').html(s.stavesPerWidth());
    $('#display_staves_per_length').html(s.stavesPerLength());

    $('#display_outer_dimension').html(s.outerDimension().toFixed(3) + '"');
    $('#display_inner_dimension').html(s.innerDimension().toFixed(3) + '"');
    $('#display_joint_angle').html(s.jointAngle().toFixed(2) + '&deg;');
    $('#display_bevel_angle').html(s.bevelAngle().toFixed(2) + '&deg;');
    $('#display_board_length').html(s.boardLengthRequired().toFixed(3) + '"');
    $('#display_board_feet').html(s.boardFeetRequired().toFixed(3) + "'");
